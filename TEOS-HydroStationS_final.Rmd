...
TS_winterplot=
 Winter_data%>%
  ggplot()+
  geom_point(aes(x=T_Cons_gsw,y= Sal1),size=1,alpha=0.7)+
  xlab('Temperature (°C)')+
  ylab('Salinity (PSS)')+
  scale_x_continuous(limits=c(1,24.00017))+
  scale_y_continuous(limits=c(34.5,36.9))+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(color="black"),
        text = element_text(size=13))+
  geom_smooth(aes(x=T_Cons_gsw, y=Sal1),method = "lm")
TS_winterplot
```


```{r}
TS_winterplotlm = lm(Sal1~T_Cons_gsw,data =  Winter_data) 
...
TS_summerplot=
 Summer_data%>%
  ...
  scale_y_continuous(limits=c(34.0,37.5))+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(color="black"),
...
```
```{r}
TS_summerplotlm = lm(Sal1~T_Cons_gsw,data =  Summer_data) 
...
```{r}
#Relationship Sal to density summer vs winter
Density_summerplot=
 Hydro_seasonal%>%
  filter(season=="summer") %>% 
  filter(Sal1>1.150) %>% 
  ggplot()+
  geom_point(aes(x=Sal1,y=sig_theta_gsw))+
  xlab('(Sal)')+
  ylab('Density')+
  scale_y_reverse()+
  scale_x_continuous(position ="bottom")
...
```
```{r}
TS_surfacewinterplot=
 Hydro_seasonal%>%
  filter(season=="winter") %>% 
  filter(Depth<=100) %>% 
  filter(Sal1>1.150) %>% 
  ggplot()+
  geom_point(aes(x=T_Cons_gsw,y= Sal1))+
  xlab('Temp')+
  ylab('Sal')+
  scale_y_reverse()+
  scale_x_continuous(position ="bottom")
